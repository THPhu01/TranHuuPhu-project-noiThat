    ...
    $(document).ready(function() {
        ...
        $('.btn-add-to-cart .button').on('click', function(e) {
            ...
            setTimeout(function(){ 
                ...
                btn_atc.closest('div').append('<a href="/giohangs" class="added-to-cart product-btn" title="View cart" tabindex="0">View cart</a>'); 
                
                // Display message
                $('body').append('<div class="cart-product-added"><div class="added-message">Sản phẩm đã được thêm vào giỏ hàng!</div>');
                setTimeout(function() {
        ...
        $('#cart-update').on('click', function(e) {
            ...
            var url = "/giohangs/"+id;
            var total =  0;
            var values = $("input[name='cartDetail[]']")
              .map(function(){
                total+=  $(this).data('price') * $(this).val()
           
                let number = $(this).data('price') *  $(this).val()
                
                $('#subtotal-'+$(this).data('value')).html(addCommas(number) + ' VND')
                return {
                    chi_tiet_don_hang_id: $(this).data('value'),
                    so_luong: $(this).val()
                }}).get();
            $.ajax({
                url: url,
                type: 'PATCH',
                data: {_token:_token, chiTietDonHangs: values},
                success: function(data) {
                    
                }
            });
            $("#subTotal").html(addCommas(total) + ' VND');
            $("#total").html(addCommas(total) + ' VND');

            // btn_atc.addClass('loading');
            setTimeout(function(){ 
                
                // Display message
                $('body').append('<div class="cart-product-added"><div class="added-message">Giỏ hàng đã được cập nhật!</div>');
                setTimeout(function() {
                    $('.cart-product-added').remove();
                }, 2000)
            }, 1000);
